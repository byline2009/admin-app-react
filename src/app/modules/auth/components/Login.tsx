    ...
    onSubmit: (values, {setStatus, setSubmitting}) => {
      ...
      setTimeout(() => {
        login(values.email, values.password)
          .then((response: any) => {
            setLoading(false)
            dispatch(auth.actions.login(response.access))
          })
          .catch((e: any) => {
            setLoading(false)
            setSubmitting(false)
  ...
  return (
    <form className='form w-100' onSubmit={formik.handleSubmit} noValidate id='kt_login_signin_form'>
      ...
      {formik.status ? (
        ...
        <div className='mb-10 bg-light-info p-8 rounded'>
          <div className='text-info'>
            Use account <strong>admin@admin.com</strong> and password <strong>demo</strong> to continue.
          </div>
        </div>
      )}
      ...
      <div className='text-center'>
        ...
        {/* begin::Google link */}
        <a href='#' className='btn btn-flex flex-center btn-light btn-lg w-100 mb-5'>
          <img alt='Logo' src={toAbsoluteUrl('/media/svg/brand-logos/facebook-4.svg')} className='h-20px me-3' />
        ...
        <a href='#' className='btn btn-flex flex-center btn-light btn-lg w-100'>
          <img alt='Logo' src={toAbsoluteUrl('/media/svg/brand-logos/apple-black.svg')} className='h-20px me-3' />
          Continue with Apple
        </a>
        {/* end::Google link */}
      </div>
